...
- **Navigation**: The app is an OS of **apps**, each grouping related sections (`src/lib/config/apps.ts`; see `docs/adr/0008-apps-as-section-umbrellas.md`). A floating bottom dock (`AppDock`) holds the home page (`/dashboard`), pinned and recent apps, and the Launchpad — an overlay grid of apps plus account actions, which replaces a full menu; there is no permanent sidebar or top bar. An app with several sections shows them in a `SectionTabs` navbar rendered by the `(app)` layout. A new section is a `menuItems` entry listed in exactly one app's `sections`. While a page registers a bulk action, the dock swaps for `FloatingActionBar` (same `DOCK_SURFACE` style).
