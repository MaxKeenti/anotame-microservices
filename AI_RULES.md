...
- **Navigation**: The app is an OS of **apps**, each grouping related sections (`src/lib/config/apps.ts`; see `docs/adr/0008-apps-as-section-umbrellas.md`). A floating bottom dock (`AppDock`) holds the home page (`/dashboard`), pinned and recent apps, and the Launchpad — an overlay grid of apps plus account actions, which replaces a full menu. From `md` up, a macOS-style `MenuBar` sits above the content (logo menu, the current app's sections, a "Go" menu of apps, search, clock, account); phones have no top bar. `CommandPalette` (⌘K / Ctrl+K) searches apps, sections, and quick actions. There is no permanent sidebar. From 1024px up (tablets included) apps open in desktop windows (`docs/adr/0009-desktop-windows.md`); shell navigation goes through `navigate()` from `$lib/desktop/navigate`.
- **Section navigation**: Apps move between sections with a tab bar, or a System Settings–style sidebar when the app sets `nav: 'sidebar'` in `apps.ts` (Ajustes). `SectionFrame` renders either one and picks the layout by container width, so it also adapts inside desktop windows.
- **Object storage**: Files go in the Railway bucket `anotame-assets` (one per environment), accessed server-side through `$lib/server/assets-bucket` (`S3_*` variables) and served to browsers with presigned URLs; keep per-user objects under `users/<id>/`.
- **Route access in pages**: Dashboard pages and the components they render read params and navigate with `useRoute()` from `$lib/desktop/route-context.svelte` (`route.params`, `route.url`, `route.goto`, `route.replaceUrl`), never `page` from `$app/state`/`$app/stores` or `goto` from `$app/navigation` — the page may be inside a window. Plain links need nothing special. Enforced by `bun run lint:routes`. An app with several sections shows them in a `SectionTabs` navbar rendered by the `(app)` layout. A new section is a `menuItems` entry listed in exactly one app's `sections`. While a page registers a bulk action, the dock swaps for `FloatingActionBar` (same `DOCK_SURFACE` style).
